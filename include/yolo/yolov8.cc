...
int inference_yolov8_model(rknn_app_context_t *app_ctx, image_buffer_t *img, object_detect_result_list *od_results)
{
    ...
    if (app_ctx->io_num.n_output == 9) {
        extern int post_process_crack(rknn_output*, rknn_app_context_t*, int, int,
                                      float, float, object_detect_result_list*);
        post_process_crack(outputs, app_ctx, app_ctx->model_width, app_ctx->model_height,
                           box_conf_threshold, nms_threshold, od_results);
    } else if (app_ctx->io_num.n_output == 3 || app_ctx->io_num.n_output == 4) {
        post_process(app_ctx, outputs, &letter_box, box_conf_threshold, nms_threshold, od_results);
    } else {
        printf("warning: model has %d outputs, skip post_process\n",
